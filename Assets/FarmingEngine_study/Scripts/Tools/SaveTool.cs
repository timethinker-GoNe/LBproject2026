        ...
        public static T LoadFile<T>(string filename) where T : class
        {
            ...
            if (IsValidFilename(filename) && File.Exists(fullpath))
            {
                ...
                catch (System.Exception)
                {
                    ...
                    if (data != null)
                        Debug.Log("[SaveTool] Loaded legacy binary save: " + filename + ". Will convert to JSON on next save.");
                    else
                        Debug.LogWarning("[SaveTool] Failed to load save file: " + filename);
                }
            }
            return data;
        }
...
#pragma warning restore SYSLIB0011
            ...
            {
                Debug.LogWarning("[SaveTool] Binary fallback also failed: " + e.Message);
        ...
        public static void SaveFile<T>(string filename, T data) where T : class
        {
            if (IsValidFilename(filename))
            {
                ...
                catch (System.Exception e) { Debug.LogWarning("[SaveTool] Error Saving Data: " + e.Message); }
            }
        }
