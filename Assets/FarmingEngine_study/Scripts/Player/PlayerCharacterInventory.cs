...
namespace FarmingEngine
{
    ...
    {
        public int inventory_size = 10; //1~9,0 키 매핑용 10칸 숏컷바
        public StartingItem[] starting_items;
        ...
        private void Start()
        {
            ...
            if (!has_inventory)
            {
                ...
                foreach (StartingItem entry in starting_items)
                {
                    if (entry.item == null) continue;
                    for (int i = 0; i < entry.quantity; i++)
                        invdata.AddItem(entry.item.id, 1, entry.item.durability, UniqueID.GenerateUniqueID());
                }
            }
        }
        ...
        public void GainItem(InventoryData inventory, ItemData item, int quantity, Vector3 source_pos)
        {
            if (item != null)
            {
                if (inventory.CanTakeItem(item.id, quantity))
                {
                    ...
                    FarmingEvents.OnItemCollected?.Invoke(item.id, quantity);
                }
                else
                {
