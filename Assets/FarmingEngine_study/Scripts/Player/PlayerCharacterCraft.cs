...
namespace FarmingEngine
{
    ...
    public class PlayerCharacterCraft : MonoBehaviour
    {
        ...
        private LineRenderer plant_indicator = null;

        public static PlayerCharacterCraft Instance;
        ...
        {
            if (plant_indicator != null)
                Destroy(plant_indicator.gameObject);
        }
        ...
        {
            bool showing = current_buildable != null && current_build_data is PlantData;
            if (showing)
            {
                current_buildable.SetBuildVisible(false);
                bool valid = current_buildable.CheckIfCanBuild();
                DrawPlantIndicator(current_buildable.transform.position, valid);
            }
            else
            {
                HidePlantIndicator();
        ...
        private void DrawPlantIndicator(Vector3 pos, bool valid)
        {
            if (plant_indicator == null)
            {
                var go = new GameObject("PlantIndicator");
                plant_indicator = go.AddComponent<LineRenderer>();
                plant_indicator.loop = true;
                plant_indicator.widthMultiplier = 0.04f;
                plant_indicator.useWorldSpace = true;
                plant_indicator.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
                plant_indicator.receiveShadows = false;
                plant_indicator.material = new Material(Shader.Find("Sprites/Default"));
                const int segments = 16;
                plant_indicator.positionCount = segments;
            }

            Color c = valid ? Color.green : Color.red;
            plant_indicator.startColor = c;
            plant_indicator.endColor = c;
            plant_indicator.enabled = true;
            ...
            for (int i = 0; i < seg; i++)
            {
                ...
                pts[i] = new Vector3(pos.x + Mathf.Cos(angle) * radius, y, pos.z + Mathf.Sin(angle) * radius);
            }
            plant_indicator.SetPositions(pts);
        }

        private void HidePlantIndicator()
        {
            if (plant_indicator != null)
                plant_indicator.enabled = false;
        }
        ...
        public Craftable CraftCraftable(CraftData data, bool skip_cost = false)
        {
            ...
            if (item != null)
                ...
                return CraftItem(construct.take_item_data, skip_cost);
            else if (plant != null)
            ...
            if (item != null && current_buildable != null && CanCraft(item, build_pay_slot, true))
            {
                ...
                if (current_buildable.CheckIfCanBuild())
                {
                    ...
                    if (item is PlantData)
                        FarmingEvents.onPlantedSeed?.Invoke(character);
