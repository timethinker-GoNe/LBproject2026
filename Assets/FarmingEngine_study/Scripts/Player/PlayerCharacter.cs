...
namespace FarmingEngine
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        private PlayerCharacterRun character_run;
        ...
        void Awake()
        {
            ...
            character_run = GetComponent<PlayerCharacterRun>();
        ...
        private void UpdateControls()
        {
            ...
            if (!panel_focus && !is_busy)
            {
                ...
                //Press Run
                if (character_run != null)
                {
                    if (controls.isPressRun())
                        character_run.Run();
                    else
                        character_run.StopRun();
                }

                //Press Build Rotate
                if (character_craft != null && character_craft.IsBuildMode())
                {
                    var currentBuildable = character_craft.GetCurrentBuildable();
                    if (currentBuildable != null)
                    {
                        float rotate_angle = currentBuildable.rotate_angle;

                        if (controls.isPressBuildRotateLeft())
                            currentBuildable.RotateManually(-rotate_angle);
                        else if (controls.isPressBuildRotateRight())
                            currentBuildable.RotateManually(rotate_angle);
                    }
                }
            }
        ...
        private void OnClick(Vector3 pos, Selectable selectable)
        {
            ...
            else if (target != null && (character_combat.HasRangedWeapon() || character_combat.IsAttackTargetInRange(target))
                && Combat.CanAutoAttack(target))
            {
                selectable.Select();
                Attack(target);
            }
            //Interact with clicked selectable (only if in range — movement is WASD only)
            else if (selectable != null && selectable.IsInUseRange(this))
            {
                selectable.Select();
        ...
        private void OnMouseHold(Vector3 pos)
        {
            ...
            // Mouse hold does not trigger movement — use WASD
        }
        ...
        public float GetMoveSpeed()
        {
            ...
            float speed = is_run ? character_run.run_speed : move_speed;
            float base_speed = IsSwimming() ? character_swim.swim_speed : speed;
        ...
        public PlayerCharacterRun Running
        {
            get { return character_run; }
        }
