...
namespace FarmingEngine
{
    ...
    public class ItemSlot : UISlot
    {
        ...
        private Sprite override_sprite;
        ...
        public void SetSlot(CraftData item, int quantity, bool selected=false)
        {
            if (item != null)
            {
                ...
                this.durability = 0f;
                override_sprite = IconOverrideLoader.GetOverride(item.id);
                icon.sprite = override_sprite ?? item.icon;
                icon.enabled = true;
        ...
        {
            return override_sprite ?? (item != null ? item.icon : null);
        }
