...
namespace FarmingEngine
{
    ...
    public class UISlotPanel : UIPanel
    {
        ...
        private void TryApplySlotAreas()
        {
            var areas = GetComponentsInChildren<SlotArea>(true);
            var panelRt = GetComponent<RectTransform>();
            foreach (var area in areas)
                area.ApplyLayout(panelRt);
        }
        ...
        private void TryApplySlotLayout()
        {
            ...
            _layoutApplied = true;
            // 자식에 SlotArea가 있으면 per-area 경로에 위임, 기존 단일-area 경로 건너뜀
            if (GetComponentsInChildren<SlotArea>(true).Length > 0) return;
        ...
        private void ApplySlotLayout()
        {
            ...
            Debug.Log($"[SlotLayout] {gameObject.name} | container={container.name} | contRect={contW:F1}x{contH:F1} | cols={cols} gap={gap} pad={pad} L={padLeft} R={padRight} T={padTop} B={padBottom} justify={justify} align={align}");
