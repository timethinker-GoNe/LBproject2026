...
namespace FarmingEngine
{
    ...
    public class Plant : Craftable
    {
        ...
        public float destruct_dropitem_percentage;  // 파괴 시 아이템 드랍 확률
        ...
        public void Harvest(PlayerCharacter character)
        {
            if (fruit != null && has_fruit && character.Inventory.CanTakeItem(fruit, 1))
            {
                ...
                {
                    if (IsDrop_DestructItem())
                        destruct.Kill();
                    else
                        destruct.Kill_NoDrop();
                }
        ...
        public bool IsDrop_DestructItem()
        {
            float randomRange = Random.Range(0f, 100f);
            return randomRange <= destruct_dropitem_percentage;
        }
