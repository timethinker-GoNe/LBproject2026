...
public class GrassComputeScript : MonoBehaviour, ISerializationCallbackReceiver
{
    ...
    // Serialization helper arrays to preserve ALL data properly
    [SerializeField, HideInInspector]
    private Vector3[] _serializedPositions;
    [SerializeField, HideInInspector]
    private Vector3[] _serializedNormals;
    [SerializeField, HideInInspector]
    private Vector2[] _serializedLengths;
    [SerializeField, HideInInspector]
    private Vector3[] _serializedColors;
    
    public void OnBeforeSerialize()
    {
        // Store ALL grass data separately before serialization
...
#if UNITY_EDITOR
            Debug.Log($"[GrassCompute] OnBeforeSerialize: Saved {count} grass data, first color: {_serializedColors[0]}");
#endif
        }
    }
    ...
    {
        // Restore ALL grass data after deserialization
...
#if UNITY_EDITOR
            Debug.Log($"[GrassCompute] OnAfterDeserialize: Restored {count} grass data, first color: {grassData[0].color}");
#endif
        }
    }
    ...
    private uint[] argsBufferReset = new uint[5]
   {
    ...
    private void OnValidate()
    {
        ...
        // Force scene to be marked as dirty when grass data changes
        // This ensures color data is saved properly after domain reload
        if (grassData != null && grassData.Count > 0)
        {
    ...
    private void OnEnable()
    {
        ...
        // Force restore serialized data on enable (fixes domain reload issues)
    ...
    {
        // Extra safety: restore data when component enables (after domain reload)
        if (_serializedPositions != null && _serializedPositions.Length > 0 && 
            grassData != null && grassData.Count == _serializedPositions.Length)
        {
            for (int i = 0; i < grassData.Count; i++)
            {
                GrassData data = grassData[i];
                
                // Only restore if data looks corrupted (black color)
            ...
            {
                Debug.Log($"[GrassCompute] RestoreSerializedData on OnEnable: First grass color = {grassData[0].color}");
            }
#endif
        }
    }
    ...
    void MainSetup(bool full)
    {
        ...
        // Validate kernel was found
        if (m_IdGrassKernel < 0)
        {
            Debug.LogError("Grass: Failed to find 'Main' kernel in compute shader. Check GrassBlades.compute");
            return;
        }
        ...
        // Validate threadGroupSize
        if (threadGroupSize == 0)
        {
            Debug.LogError("Grass: Failed to get thread group size from compute shader. Check if compute shader is valid.");
            threadGroupSize = 128; // Fallback to default value from compute shader
        }
    ...
    void GetFrustumData()
    {
        ...
        // Validate camera transform exists
        if (m_MainCamera.transform == null)
        {
            return;
        }
    ...
    private void Update()
    {
        ...
        if (threadGroupSize == 0)
        {
            Debug.LogWarning("Grass: threadGroupSize is 0, skipping dispatch");
            return;
        }
    ...
    private void SetGrassDataBase(bool full)
    {
        ...
        // Color Variation
        m_InstantiatedComputeShader.SetFloat("_ColorVariationProbability", currentPresets.colorVariationProbability);
        m_InstantiatedComputeShader.SetFloat("_ColorVariationNoiseScale", currentPresets.colorVariationNoiseScale);
        m_InstantiatedComputeShader.SetFloat("_ColorVariationThreshold", currentPresets.colorVariationThreshold);
        m_InstantiatedComputeShader.SetVector("_VariationBottomTint", currentPresets.variationBottomTint);
        m_InstantiatedComputeShader.SetVector("_VariationTopTint", currentPresets.variationTopTint);
        
        // Brightness Variation
        m_InstantiatedComputeShader.SetFloat("_BrightnessVariationScale", currentPresets.brightnessVariationScale);
        m_InstantiatedComputeShader.SetInt("_BrightnessVariationSteps", currentPresets.brightnessVariationSteps);
        m_InstantiatedComputeShader.SetFloat("_BrightnessVariationStrength", currentPresets.brightnessVariationStrength);
    }
...
// Compute Buffer still works correctly with default layout
